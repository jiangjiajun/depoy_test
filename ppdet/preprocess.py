...
class Resize(object):
    ...
    def __call__(self, im, im_info):
        ...
        im_info['resize_shape'] = [
            round(im_scale_x * im.shape[0]), round(im_scale_y * im.shape[1])]
        if np.fabs(im_scale_y - im_scale_x) < 1e-06:
            tmp2 = round(im_scale_y * im.shape[1]) / im.shape[1]
            tmp1 = tmp2
        else:
            tmp1 = round(im_scale_x * im.shape[0]) / im.shape[0]
            tmp2 = round(im_scale_y * im.shape[1]) / im.shape[1]
        if self.use_cv2:
            im = cv2.resize(
                im,
